...
fn query_packet(unicast: bool, types: &[DnsRecordType], fqdn: &DnsName) -> Result<Vec<u8>, std::io::Error> {
...
impl Discovery {
	...
	pub async fn single_query(
		...
		fqdn: &DnsName,
		timeout: Duration,
		mut filter: impl FnMut(SocketAddr, &DnsResponse) -> bool,
	) -> Result<DnsResponse, MultiIpIoError> {
		//let fqdn = fqdn.into_fqdn();
		let query_packet = query_packet(false, &types, &fqdn)?;
		let socket = self.socket.into_async().await?;
		if let Err(err) = socket.send_multicast(&query_packet).await {
		...
		loop {
			let ((recv_size, origin_addr), packet) = match tokio::time::timeout_at(deadline, socket_recv.recv_multicast()).await {
				Ok(Ok(recv)) => recv,
				Ok(Err(err)) => return Err(err),
				Err(_) => return Err(MultiIpIoError::IoError(std::io::Error::from(std::io::ErrorKind::TimedOut))),
			};
			if recv_size == 0 {
				continue;
			}

			let response = match DnsMessage::from_bytes(&packet[..recv_size]) {
				Ok(response) if response.message_type() == DnsMessageType::Response => DnsResponse::from(response),
				_ => continue,
			};
			if !filter(origin_addr, &response) {
				continue;
			}
			if !response.answers().iter().any(|answer| answer.name() == fqdn) {
