...
pub struct Service {
	...
	ttl: u32,
	additional_txt: Option<(DnsName, Vec<u8>)>,
}
impl Service {
	#[inline(always)]
	/// The service type.
	...
	pub fn dns_response(&self) -> Result<DnsMessage, ServiceDnsPacketBuilderError> {
		...
		if self.port==0 {
			...
			if let Some((name, data)) = self.additional_txt.as_ref() {
				response.add_additional( {
					...
					record
						...
						.set_data(Some(RData::TXT(TXT::from_bytes(vec![&data.clone()]))))
						.set_name(name.clone())
						.set_ttl(self.ttl)
						.set_mdns_cache_flush(true);
...
impl ServiceBuilder {
	...
	pub fn new(service_type: impl IntoDnsName, service_name: impl IntoDnsName, port: u16) -> Result<Self, BadDnsNameError> {
		let service_type = service_type.into_name().map_err(|_| BadDnsNameError)?;
		let service_name = service_name.into_name().map_err(|_| BadDnsNameError)?;
		let local = DnsName::from_labels(["local"]).unwrap();
		let service_id = service_name.clone().append_domain(&service_type).map_err(|_| BadDnsNameError)?;
		let service_hostname = service_name.clone().append_domain(&local).map_err(|_| BadDnsNameError)?;
		Ok(Self(Service {
			service_id,
			service_hostname,
			service_type,
